...
> **Project status.** This README is the original planning session summary and is kept as the
> historical record of the design discussion. For the current technical review of this plan, see
> [`docs/REVIEW.md`](docs/REVIEW.md). For the ordered plan to get from here to a deployed system,
> see [`docs/ROADMAP.md`](docs/ROADMAP.md). Where the two disagree with the text below, the review
...
> **Note:** the `ptz` and `record.events` blocks below do not match current Frigate's configuration
> schema and will not load. See finding C3 in [`docs/REVIEW.md`](docs/REVIEW.md) for the details and
> [`docs/ROADMAP.md`](docs/ROADMAP.md) for the corrected version.
...
See the decision log in [`docs/ROADMAP.md`](docs/ROADMAP.md) for recommendations against each of
these, including three that can be closed now.
